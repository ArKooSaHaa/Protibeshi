...
export const RentPage = () => {
  ...
  const [filteredListings, setFilteredListings] = useState(rentListings);
  ...
  const applyFilters = (currentFilters, sourceListings = allListings) => {
    ...
    results = results.filter(
      (item) => item.price >= currentFilters.minPrice && item.price <= currentFilters.maxPrice
    );

    // Filter by distance
    results = results.filter((item) => item.distance <= currentFilters.radius);

    // Filter by property type
    if (currentFilters.propertyTypes.length > 0) {
      results = results.filter((item) => currentFilters.propertyTypes.includes(item.type));
    }
    ...
    if (currentFilters.furnishing.length > 0) {
      results = results.filter((item) => currentFilters.furnishing.includes(item.furnishing));
    }

    // Filter by availability
    if (currentFilters.availability.length > 0) {
      results = results.filter((item) => currentFilters.availability.includes(item.availability));
    }
  ...
  const handleAddProperty = (newProperty) => {
    const nextId = allListings.length ? Math.max(...allListings.map((item) => item.id)) + 1 : 1;
    ...
    <div className={styles.rentPage} ref={containerRef}>
      ...
      <motion.div className={styles.header} style={{ opacity: headerOpacity, y: headerY }}>
        ...
        <div className={styles.headerContent}>
          ...
          <p className={styles.subtitle}>Verified rentals within walking distance</p>
        </div>
      </motion.div>
        ...
        >
          {filteredListings.length > 0 ? (
            ...
            <div className={styles.emptyState}>
              ...
              <p>Try adjusting your filters or widening your search radius</p>
            </div>
          )}
        </motion.section>
      </div>
