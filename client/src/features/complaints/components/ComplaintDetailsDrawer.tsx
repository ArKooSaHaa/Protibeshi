...
export const ComplaintDetailsDrawer = ({ complaint, onClose }: ComplaintDetailsDrawerProps) => { 
 
 return ( 
   <AnimatePresence> 
     {complaint && ( 
       <motion.div 
         className={styles.overlay}          initial={{ opacity: 0 }} 
         animate={{ opacity: 1 }} 
         exit={{ opacity: 0 }} 
         onClick={onClose} 
       > 
         <motion.aside 
           className={styles.drawer}            initial={{ x: 420 }} 
           animate={{ x: 0 }} 
           exit={{ x: 420 }} 
...
${styles[`status${complaint.status.replace(/\s/g, '')}`]}`}> 
             ...
             <span className={styles.priority}>{complaint.priority} 
Priority</span> 

             <span 
className={styles.metaItem}>{complaint.category}</span> 
           </div> 
           ...
           <div className={styles.section}> 
             ...
             <div className={styles.metaGrid}> 
               ...
               <div> 
                 <span>Reported</span> 
                 <strong>{new 
Date(complaint.createdAt).toLocaleDateString()}</strong> 
               </div> 
             </div> 
           </div> 
...
&& ( 
           ...
           {complaint.resolutionSummary && ( 
             <div className={styles.section}> 
               ...
               <div 
className={styles.resolutionBox}>{complaint.resolutionSummary}</div> 
             </div> 
           )} 
 
           <div className={styles.footer}> 
             <span className={styles.footerMeta}> 
               <Calendar size={14} /> Last updated {new 
