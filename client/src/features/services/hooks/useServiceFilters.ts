...
import { ServiceFilterState, ServiceItem } from '../types/service.types';
...
const applySort = (services: ServiceItem[], sortBy: ServiceFilterState['sortBy']) => {
  switch (sortBy) {
    case 'Nearest':
...
export const useServiceFilters = (services: ServiceItem[], filters: ServiceFilterState) => {
  return useMemo(() => {
    const filtered = services.filter((service) => {
      ...
      if (filters.availability.length > 0 && !filters.availability.includes(service.availability)) {
        return false;
      }
