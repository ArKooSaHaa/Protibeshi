...
import { CheckCircle2, Clock, Flag, MapPin, Users, X } from 'lucide-react';
import type { ReliefRequest } from '../types/relief.types';
import { formatDistance, formatRelativeTime, statusConfig, urgencyConfig } from '../utils/relief.utils';
...
export const ReliefDetailsDrawer = ({ request, onClose }: ReliefDetailsDrawerProps) => {
  return (
    <AnimatePresence>
      {request && (
        <>
          ...
          >
            ...
            <div className={styles.header}>
              <div className={styles.headerTop}>
                <div className={styles.titleBlock}>
                  <div className={styles.badges}>
                    <span className={styles.helpTypeBadge}>{request.helpType}</span>
                    <span className={`${styles.urgBadge} ${styles[urgencyConfig[request.urgency].colorClass]}`}>
                      <span className={`${styles.dot} ${styles[urgencyConfig[request.urgency].dotClass]}`} />
                      {urgencyConfig[request.urgency].label}
                    </span>
                  </div>
                  <h2 className={styles.drawerTitle}>{request.title}</h2>
                </div>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, flexShrink: 0 }}>
                  <span className={`${styles.statusBadge} ${styles[statusConfig[request.status].colorClass]}`}>
                    {request.status}
                  </span>
              ...
              <div className={styles.meta}>
                ...
                <span className={styles.metaItem}>
                  <Clock size={11} />{formatRelativeTime(request.createdAt)}
                </span>
                <span className={styles.metaItem}>
                  <Users size={11} />{request.volunteerCount} volunteer{request.volunteerCount !== 1 ? 's' : ''}
                </span>
                {request.verified && (
            ...
            <div className={styles.body}>
                  ...
                  {request.timeline.map((entry, idx) => (
                    <div key={idx} className={styles.timelineItem}>
                      ...
                      <div className={styles.timelineContent}>
                        <span className={styles.timelineStage}>{entry.stage}</span>
                  ...
                  {request.volunteers.length === 0 ? (
                    ...
                    request.volunteers.map((v) => (
                      <div key={v.id} className={styles.volunteerItem}>
                        <div className={styles.avatar}>{v.avatarInitials}</div>
                        <div>
                  ...
                  {request.comments.length === 0 ? (
                    ...
                    request.comments.map((c) => (
                      <div key={c.id} className={styles.commentItem}>
                        <div className={styles.commentAvatar}>{c.avatarInitials}</div>
              ...
              {request.resolutionSummary && (
                <div className={styles.section}>
                  ...
                  <div className={styles.resolutionBox}>{request.resolutionSummary}</div>
                </div>
              )}
            </div>
