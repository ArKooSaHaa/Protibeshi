          ...
          >
            ...
            <div className={styles.body}>
              ...
              <div className={styles.section}>
                <span className={styles.sectionLabel}>Type of Help</span>
                <div className={styles.chips}>
                  {reliefHelpTypes.map((type) => (
                    <button
                      key={type}
                      className={`${styles.chip} ${filters.helpTypes.includes(type) ? styles.selected : ''}`}
              ...
              {/* Urgency */}
              <div className={styles.section}>
                <span className={styles.sectionLabel}>Urgency Level</span>
                <div className={styles.chips}>
                  {reliefUrgencyLevels.map((u) => {
                    const isSelected = filters.urgencies.includes(u);
                    const colorClass = isSelected ? (urgencyColors[u] || styles.selected) : '';
                    return (
                      <button
                        key={u}
              ...
              <div className={styles.section}>
                ...
                <div className={styles.chips}>
                  {reliefStatuses.map((s) => (
                    <button
                      key={s}
                      className={`${styles.chip} ${filters.statuses.includes(s) ? styles.selected : ''}`}
                ...
                <div className={styles.sliderRow}>
                  <div className={styles.sliderLabel}>
                    ...
                    <span className="font-semibold text-slate-700">
                      {filters.distance < 1000 ? `${filters.distance}m` : `${(filters.distance / 1000).toFixed(1)}km`}
                    </span>
                    <span>1km</span>
                  </div>
                  <input
              ...
              <div className={styles.section}>
                ...
                <div className={styles.chips}>
                  {reliefTimeRanges.map((r) => (
                    <button
                      key={r}
                      className={`${styles.chip} ${filters.timeRange === r ? styles.selected : ''}`}
              ...
              <div className={styles.section}>
                ...
                <div
                  className={`${styles.toggle} ${filters.verifiedOnly ? styles.active : ''}`}
                  onClick={() => onSetVerifiedOnly(!filters.verifiedOnly)}
                >
                  <div className={styles.toggleText}>
                    <span className={styles.toggleLabel}>Verified neighbors only</span>
                    <span className={styles.toggleDesc}>Show only posts from verified residents</span>
                  </div>
                  <div className={`${styles.toggleSwitch} ${filters.verifiedOnly ? styles.on : ''}`}>
            ...
            <div className={styles.footer}>
              ...
              <button className={styles.btnApply} onClick={onClose} type="button">
                Apply Filters
              </button>
            </div>
          </motion.div>
